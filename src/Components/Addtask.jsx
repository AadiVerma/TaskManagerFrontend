...
export default function Addtask({ setAddTask ,setTaskAdded
}) {
  const [date,setDate]=useState('');
  const [heading,setHeading]=useState('');
  const [para,setPara]=useState('');
  const [color, setColor] = useState("#D075FF");
  ...
  return (
      <div ref={modelref} onClick={closeModel} className="bg-transparent bg-opacity-30 backdrop-blur-sm inset-0 flex justify-center fixed items-center">
        <div style={{ backgroundColor: `${color}` }} className="p-8 rounded-3xl w-2/5">
          ...
          <h1 style={{ color: (color === '#FBDB3A' || color === '#98FF73') ? "#48484B" : "white" }} className="text-3xl font-semibold ">Create a task</h1>
          </div>
          <input onChange={(e)=>setHeading(e.target.value)}style={{ color: (color === '#FBDB3A' || color === '#98FF73') ? "#48484B" : "white", borderColor: (color === '#FBDB3A' || color === '#98FF73') ? "#48484B" : "white", }} type="text" placeholder="Heading" className={`${color === '#FBDB3A' || color === '#98FF73' ? "placeholder:text-slate-800" : "placeholder:text-white"}  rounded-lg p-4 bg-transparent text-white  outline-none border-2 border-slate-100 block m-2  mb-6 w-full font-bold `} />
          <textarea onChange={(e)=>setPara(e.target.value)}style={{ color: (color === '#FBDB3A' || color === '#98FF73') ? "#48484B" : "white", borderColor: (color === '#FBDB3A' || color === '#98FF73') ? "#48484B" : "white" }} placeholder="Description" className={`rounded-lg p-4 min-h-24 bg-transparent text-white ${color === '#FBDB3A' || color === '#98FF73' ? "placeholder:text-slate-800" : "placeholder:text-white"} outline-none border-2 border-slate-100 block m-2  w-full font-semibold`} />
          ...
          <input id={"deuDate"} onChange={(e)=>setDate(e.target.value)}style={{ color: (color === '#FBDB3A' || color === '#98FF73') ? "#48484B" : "white", borderColor: (color === '#FBDB3A' || color === '#98FF73') ? "#48484B" : "white", }} type="date" className={`w-full bg-transparent border-2 font-bold rounded-lg p-4 m-2 outline-none ${color === '#FBDB3A' || color === '#98FF73' ? "placeholder:text-slate-800" : "placeholder:text-white"} `} />
          ...
          <div className="flex justify-center gap-2">
            <button className="w-2/5 bg-red-500 p-2 hover:bg-red-600 text-slate-100 text-lg rounded-lg font-semibold" onClick={() => setAddTask(false)}>Cancel</button>
            <button className="w-2/5 bg-green-500 hover:bg-green-600 text-slate-100 text-lg p-2 rounded-lg font-semibold" onClick={Savehandler}>Save</button>
          </div>
        </div>
    </div>
  )
